...
///   • **local**  — the watch started the session, so its `PulseArcCore.TimerEngine` drives and
...
final class WatchTimerViewModel: ObservableObject {
    ...
    private let core: PulseArcCore.TimerEngine
    private let connectivity = WatchConnectivityManager.shared
    private var revision: UInt64 = 0
    private var cancellables = Set<AnyCancellable>()

    init(configuration: TimerConfiguration = .default) {
        let core = PulseArcCore.TimerEngine(configuration: configuration)
        self.core = core
