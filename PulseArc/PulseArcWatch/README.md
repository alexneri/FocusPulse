# PulseArcWatch — Apple Watch companion (Epic 7)

Source for the PulseArc Apple Watch app. Like `PulseArcWidgets/`, this folder is **not** part
...
verified `PulseArcCore` domain and the shared theme/app-group types the phone app already uses.

## What's here

| File | Story | Role |
|------|-------|------|
| `PulseArcWatchApp.swift` | 7.1 | `@main` watchOS App; injects `ThemeStore` + `WatchTimerViewModel`. |
...
| `WatchTimerViewModel.swift` | 7.2–7.6 | Adapter over `PulseArcCore.TimerEngine`; renders from one `TimerSnapshot`; local-drive vs mirror; routes taps via `TimerAuthority`. |
| `WatchConnectivityManager.swift` | 7.4 | `WCSession` transport (watch side); defers all decisions to `PulseArcCore`. |
...
dedupe — lives in `PulseArcCore` (`Services/WatchSync.swift`) and is unit-tested
(`WatchSyncTests`, `swift test`). The iPhone half is `PulseArc/Connectivity/PhoneConnectivityManager.swift`
(already compiled into the app).

## Xcode setup (needs your Apple account for signing + App Group)

1. **Add a watchOS App target** `PulseArcWatch` — File ▸ New ▸ Target ▸ watchOS ▸ App.
   - Bundle id `moe.sei.PulseArc.watchkitapp`, watchOS **11.0** min, SwiftUI lifecycle.
   - Delete the stub `ContentView`/`App` Xcode generates; add the files in this folder instead.
2. **Add the PulseArcCore package** to the watch target: target ▸ General ▸ Frameworks ▸ `+` ▸
   `PulseArcCore`.
3. **Share source via Target Membership** — select each of these (already in the app) and tick the
   ...
   - `PulseArc/Theme/VisualTheme.swift`, `Theme/ThemeStore.swift`, `Theme/Color+Hex.swift`
   - `PulseArc/Shared/SharedTimerState.swift`
4. **App Group** — add `group.moe.sei.PulseArc` to the watch target's Signing & Capabilities
...
6. **Complications** — add a **Widget Extension** target embedded in the watch app
   (`moe.sei.PulseArc.watchkitapp.complications`), add `WatchComplications.swift` to it, and give
