...
// NOTE: PulseArc Apple Watch complications (Story 7.5). Ship these in a Widget Extension embedded
...
@main
struct PulseArcWatchComplications: WidgetBundle {
    var body: some Widget { FocusComplication() }
}

struct FocusComplication: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: "PulseArcWatchComplication", provider: WatchComplicationProvider()) { entry in
...
struct ComplicationView: View {
    ...
    private var inline: some View {
        Label(
            running ? "\(CDisplay.name(state)) \(CDisplay.mmss(state.remainingSeconds))" : "PulseArc · Ready",
            systemImage: glyph
        )
    }
    private var rectangular: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(running ? CDisplay.name(state) : "PulseArc").font(.headline)
