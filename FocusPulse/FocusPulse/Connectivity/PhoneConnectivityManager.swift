...
/// iPhone side of the FocusPulse ↔ Apple Watch state bridge (Story 7.4 / architecture §8.3).
///
/// This is a thin WatchConnectivity **transport**. Every decision — who owns the session, which
/// snapshot wins a conflict — is delegated to the verified `FocusPulseCore` policy
...
/// `onCommandFromWatch` to receive. See `FocusPulseWatch/README.md` for the end-to-end wiring.
final class PhoneConnectivityManager: NSObject {
    ...
    /// deterministically.
    func publish(
        state: FocusPulseCore.TimerState,
        sessionType: FocusPulseCore.SessionType,
